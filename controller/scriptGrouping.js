script = [
    ...
    {
        "text": " 딥시크가 밝힌 AI 모델 개발비는 85원, gpt4 개발비의 18분의 1 메탈 라마쓰리의 1/10의 불가합니다. 실제 비용은 더 될 거란 추 정도 나오지만 신선한 충격임은 분명합니다. 가능성을 우리산업계의 화두를 던져 준 거 같습니다. 예상을 뛰어넘은 딥시크 모델의 추론 능력에 국내 기업들의 움직임이 분주해졌습니다. 라희냐후 지분 매각 압박에도 모습을 드러내지 않던 이혜진 네이버 창업자는 다음 달 7년 만에 경영으로 복귀합니다. AI 비전을 제시하지 못하고 있던 카카오는 오픈 ai와 손잡기로 했습니다. 한국기업이 개발한 AI 모델을 오픈 소스로 공개해 외부 개발자들의 활용을 독려했어야 했다는 탄식도 나왔습니다. 한국 업체들은 구글 MS 메타만큼 막대한 개발 비용을 투자하지 못하면서 오픈 ai처럼 폐쇄적인 환경을 구축하다 보니 이도저도 아닌게 됐다는 겁니다.",
        "words": [
            {
                "word": "▁",
                "startTime": 19.7,
    ...
    {
        "text": "문제는 AI 개발의 필수인 고성능 그래픽 처리장치 gpu를 확보하는 겁니다. 정부는 현재 2000장인 gpu를 2027년까지 선만장으로 늘리겠다고 밝혔습니다. 전 세계에서 수요가치솟은 집 비율을 어떻게 구할지? 계산은 어떻게 마련할지 관리 체계를 어떻게 갖출지 더 많은 과제가 따라붙었습니다. MBC 뉴스 김유미입니다.",
        "words": [
            {
                "word": "▁",
                "startTime": 124.7,
...
function korGettimestamp(script) {
    ...
    groupedSentences.forEach((sentence) => {
        ...
        while (sentence.trim().length - 1 > temp.length) {//문장부호를 제외한 문장 길이로 일치 확인
            let word = script.words[wordindex].word;

            temp += word.replace("▁", " ");//파일로 저장된 json에서는 언더바가 해당 특수문자라 나중에 확인 필요
            wordindex++;
        }
